...
function extxyz_approx_eq(args...; kwargs...)
    # Keys ExtXYZ exposes as atomkeys, but they are no longer standard atomkeys.
    ignore_atprop = [:atomic_number, :atomic_symbol, :masses]
    test_approx_eq(args...; ignore_atprop, kwargs...)
end
...
@testset "ExtXYZ system write / read" begin
    ...
    mktempdir() do d
        ...
        extxyz_approx_eq(system, system2; rtol=5e-6)
    end
end
...
@testset "ExtXYZ trajectory write/read" begin
    ...
    mktempdir() do d
        ...
        for (system, newsystem) in zip(systems, newsystems)
            extxyz_approx_eq(system, newsystem; rtol=5e-6)
        end

        extxyz_approx_eq(systems[end], load_system(ExtxyzParser(), outfile);    rtol=5e-6)
        extxyz_approx_eq(systems[2],   load_system(ExtxyzParser(), outfile, 2); rtol=5e-6)
    end
end
